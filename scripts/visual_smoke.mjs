...
assert.match(sShapeTimText, /Co to jest:\s*Taśma LED Premium S-Shape/i);
assert.match(sShapeTimText, /Parametry produktu/i);
assert.doesNotMatch(sShapeTimText, /Opis dla TIM\.pl|Dane techniczne|Producent\s*:|EAN\s*:/i);
assert.equal(await page.locator(".parameter-section").count(), 0);
assert.equal(await page.locator(".tim-operational").count(), 1);
assert.match(await page.locator(".tim-operational").innerText(), /do weryfikacji/i);
assert.match(await page.locator(".tim-operational").innerText(), /EPREL.*nie zgadza się/i);
await page.screenshot({ path: "/tmp/prescot-s-shape-tim.png", fullPage: true });

await page.locator("#search-input").fill("00826");
await page.waitForTimeout(260);
assert.equal(await page.locator(".product-accordion").count(), 0, "Kaja nie może być widoczna w kanale TIM.");

await page.locator("#search-input").fill("5905475368349");
await page.waitForTimeout(260);
await page.locator(".product-trigger").click();
const wcobTimText = await page.locator(".description-preview").innerText();
assert.match(await page.locator(".product-trigger").innerText(), /24WCOB320WW5IP62/i);
assert.match(wcobTimText, /Parametry produktu/i);
assert.match(wcobTimText, /Indeks handlowy:\s*24WCOB320WW5IP62/i);
assert.doesNotMatch(wcobTimText, /Opis dla TIM\.pl|Dane techniczne|Producent\s*:|EAN\s*:|kod produktu|numer katalogowy/i);
assert.equal(await page.locator(".parameter-section").count(), 0);
assert.match(await page.locator(".tim-operational").innerText(), /opis gotowy do mapowania/i);
await page.locator('[data-action="edit"]').click();
await page.locator('[data-action="save-edit"]').click();
assert.match(await page.locator("#buffer-state").innerText(), /1 zmiana/i);
assert.equal(await page.locator("#export-edits").isDisabled(), false);
await page.locator('[data-action="edit"]').click();
await page.locator('[data-action="reset-edit"]').click();
assert.match(await page.locator("#buffer-state").innerText(), /0 zmian/i);
...
assert.match(economicBlueTimText, /Do oświetlenia liniowego lub dekoracyjnego/i);
assert.match(economicBlueTimText, /osobę z odpowiednimi kwalifikacjami/i);
assert.match(economicBlueTimText, /Co to jest:\s*Taśma Economic IP63 12V 60led niebieska SMD2835 \(5\)/i);
assert.match(economicBlueTimText, /Parametry produktu/i);
assert.doesNotMatch(economicBlueTimText, /5901885261386|Opis dla TIM\.pl|Dane techniczne|Producent\s*:|EAN\s*:/i);
assert.equal(await page.locator(".description-preview h2").count(), 1);
assert.equal(await page.locator(".description-preview h3").count(), 3);
...
console.log("TIM: definicja, zastosowanie, parametry w punktach oraz zasady doboru i bezpieczeństwa; bez tabeli danych.");
console.log("EH007: bez proceduralnych instrukcji montażowych, EAN-u i indeksu wewnętrznego.");
