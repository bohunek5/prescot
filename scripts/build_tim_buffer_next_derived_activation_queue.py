#!/usr/bin/env python3
"""Build activation queue for fully completed products from the next derived batch."""
...
SOURCE_PATH = DATA / "buffer-eprel-next-derived3-queue-2026-09-01.json"
EPREL_VERIFY_PATH = DATA / "buffer-eprel-next-derived2-final-postverify-2026-09-01.json"
DOCUMENT_VERIFY_PATH = DATA / "buffer-next-derived2-documents-final-postverify-2026-09-01.json"
OUTPUT_PATH = DATA / "buffer-next-derived1-activation-queue-2026-09-01.json"
...
document_by_id = {int(row["id"]): row for row in document_verify["products"] if row.get("verified")}

items = []
rejected = []
for row in source["items"]:
    object_id = int(row["pimcoreId"])
    document = document_by_id.get(object_id)
    has_catalog_card = bool(document) and len(document["live"]["downloads"].get("dataSheet", [])) == 1
    guards = {
        "verifiedEprel": object_id in eprel_ids,
        "verifiedDocuments": bool(document),
        "catalogCard": has_catalog_card,
    ...
    if not all(guards.values()):
        rejected.append({
            ...
            "reason": "safe_catalog_card_missing" if not has_catalog_card else "guard_failed",
            "guards": guards,
        })
        continue
    items.append({
