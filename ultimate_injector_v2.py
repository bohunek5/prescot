...
def parse_cob_48v():
    ...
    parts = html.split('<div class="product-accordion"')
    items = []
    
    for p in parts[1:]:
        model_match = re.search(r'data-model="([^"]+)"', p)
        ...
        items.append('<div class="product-accordion"' + p)
    return items
...
"""
    ...
    for i, cob in enumerate(cob_items):
        # The COB items from HTML already have <span class="product-model">48EC480...
        # Let's replace the model text with numbered model
        model_name = re.search(r'data-model="([^"]+)"', cob).group(1)
        cob = re.sub(
            r'<span class="product-model">[^<]+</span>',
            f'<span class="product-model">{67 + i}. {model_name}</span>',
            cob
        )
        
        # Adjust layout: We need product-controls to be AFTER product-body.
        # But in tasmy-cob-48v-opisy.html, the structure is raw. We need to build the accordion.
        raw_html = cob[cob.find('<div class="product-body">') + len('<div class="product-body">'):]
        raw_html = raw_html[:raw_html.rfind('</div>')] # remove the accordion closing div
        
        acc = f'''<div class="product-accordion" data-model="{model_name}">
<button class="product-trigger" onclick="toggleProduct(this)">
<div class="product-info">
<span class="product-model">{67 + i}. {model_name}</span>
<span class="product-name">{model_name} (Taśma COB 48V)</span>
</div>
<svg class="chevron" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="6 9 12 15 18 9"></polyline></svg>
</button>
<div class="product-body">
<div class="model-block" id="desc-view-wapro-{model_name}">
{raw_html}
</div>
<textarea id="desc-wapro-{model_name}" style="display:none;">
{raw_html}
</textarea>
</div>
<div class="product-controls">
<button class="control-btn btn-edit" id="btn-edit-wapro-{model_name}" onclick="toggleEdit('wapro', 'tasmy', '{model_name}')">Edytuj opis</button>
<button class="control-btn btn-save" id="btn-save-wapro-{model_name}" onclick="saveDescription('wapro', 'tasmy', '{model_name}')" style="display: none;">Zapisz opis</button>
<button class="control-btn btn-copy" onclick="copyDescriptionHtml('wapro', '{model_name}', this)" style="border-color: #f59e0b; color: #f59e0b;" title="Kopiuj opis HTML">Kopiuj opis HTML</button>
<button class="control-btn btn-ean" onclick="navigator.clipboard.writeText('BRAK'); this.innerText='Skopiowano!'; setTimeout(()=>this.innerText='EAN: BRAK', 2000);" style="border-color: #0ea5e9; color: #0ea5e9;" title="Skopiuj EAN">EAN: BRAK</button>
<span class="control-status" id="status-wapro-{model_name}"></span>
</div>
</div>'''
        cob_items[i] = acc
    ...
    for i, rozd in enumerate(rozd_items):
        rozd = rozd.replace('{NUM}', str(18 + i))
        rozd_items[i] = rozd
