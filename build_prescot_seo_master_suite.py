#!/usr/bin/env python3
"""
MASTER SEO & AI COPYWRITING SUITE DLA PRESCOT & TIM (V14.0 - TUNED SHOPER & STRICT COLOR TAXONOMY)
Zgodnie z wytycznymi z PDF ("SEO + AI - Ściąga do tworzenia opisów produktów") i poleceniami Karola:
1. Warstwa 1: Co to jest? (1-2 zdania prostym, ludzkim językiem bez żargonu katalogowego).
2. Warstwa 2: Do czego się używa? (gdzie się montuje, do jakich instalacji, dla kogo - ściśle dopasowane do barwy i typu!).
3. Warstwa 3: Parametry techniczne w punktach (czytelnie, ZERO kodów, ZERO indeksów, ZERO EAN w tekście!).
4. Ścisła taksonomia kolorów taśm LED (niebieska, czerwona, zielona, żółta, różowa, bursztynowa = DEKORACJA/AKCENT, NIGDY oświetlenie główne!).
5. Integracja i tuning opisów z Shoper (sourceDescription) - wyczyszczone linki, "Economic" -> "Standard".
6. Brak nazw produktów w nagłówkach sekcji (nagłówki to wyłącznie korzyści techniczne).
7. Zgodność z TIM i WAPRO.
"""
...
def clean_text_repetitions(text):
    """Usuwa powtarzające się sąsiednie słowa i czyści podwójne marki."""
...
def clean_shoper_desc(raw_sd):
    """Czyści surowy opis z Shopera z martwych linków, zaproszeń do kliknięć i zamienia Economic na Standard."""
    if not raw_sd:
        return ""
    t = str(raw_sd)
    # Usuń HTML jeśli jest
    t = re.sub(r'<[^>]+>', ' ', t)
    # Usuń linki i odnośniki
    t = re.sub(r'Więcej informacji o produkcie:?\s*Kliknij tutaj.*', '', t, flags=re.I)
    t = re.sub(r'Kliknij tutaj.*', '', t, flags=re.I)
    t = re.sub(r'https?://\S+', '', t)
    # Zamień Economic na Standard
    t = re.sub(r'\bEconomic\b', 'Standard', t, flags=re.I)
    t = re.sub(r'\bECON\b', 'Standard', t, flags=re.I)
    # Usuń "Dla bardziej wymagających klientów polecamy serię Premium lub Delux."
...
def build_unique_product_copy(p):
    ...
    clean_mod = clean_model_name(name)
    ...
    # -------------------------------------------------------------------------
    # 0. SZYBKOZŁĄCZKI INSTALACYJNE WAGO SERII 221
    # -------------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------------
    # 1. ZŁĄCZKI DO TAŚM LED (HIPPO-M, MULTI 9-IN-1, KLIK, TAŚMA-PRZEWÓD, TAŚMA-TAŚMA)
    # -------------------------------------------------------------------------
    if any(w in uname for w in ["ZŁĄCZKA DO TAŚMY", "ZLACZKA DO TASMY", "HIPPO", "MULTI 9-IN-1", "SZYBKOZŁĄCZKA DO TAŚMY", "ZESTAW ZAŚLEPEK I KLEJU"]):
        ...
        uses = [
            "Niezbędna do szybkiego montażu i łączenia odcinków taśm LED w profilach aluminiowych, zabudowach meblowych, sufitach podwieszanych i korytkach kablowych.",
            "Idealne rozwiązanie dla instalatorów pozwalające na bezproblemowe łączenie narożników i doprowadzanie zasilania bez użycia lutownicy."
        ]
        usage = pick(uid, "zl_use", uses)
        ...
        faq = [
            ("Czy ta złączka pasuje do taśm COB?", "Tak, konstrukcja styków została zoptymalizowana pod kątem gęstych taśm COB oraz standardowych taśm SMD, zapewniając pewny kontakt na całej szerokości padów."),
            ("Czy do montażu potrzebna jest lutownica?", "Nie. Złączka jest w 100% beznarzędziowa – wystarczy wsunąć taśmę pod styki i zatrzasnąć klamrę.")
        ]

        sec_heading = "Szybki i beznarzędziowy montaż taśm LED"
        return render_full_structure(title, sec_heading, [intro_p1, intro_p2], usage, features, faq, "Akcesoria do taśm LED", uid)

    # -------------------------------------------------------------------------
    # 2. GNIAZDA, WTYKI DC, SZYBKOZŁĄCZKI KLIK I PRZEWODY
    # -------------------------------------------------------------------------
    if any(w in uname for w in ["GNIAZDO DC", "WTYK DC", "GNIAZDO 2-PIN", "WTYK 2-PIN", "SZYBKOZŁĄCZKA", "ZŁĄCZKA KLIK", "PRZEWÓD DC", "PRZEWOD DC"]):
        ...
        uses = [
            "Przeznaczone do łączenia zasilaczy LED z taśmami, sterownikami strefowymi, ściemniaczami i oprawami meblowymi.",
            "Niezastąpione w instalacjach oświetlenia szafek kuchennych, garderób, gablot ekspozycyjnych i witryn sklepowych."
        ]
        usage = pick(uid, "dc_use", uses)
        ...
        faq = [
            ("Do jakich zasilaczy pasuje to złącze?", "Złącze jest w 100% kompatybilne ze wszystkimi standardowymi zasilaczami LED wyposażonymi we wtyk lub gniazdo DC 5,5 × 2,1 mm."),
            ("Czy złącze wymaga lutowania?", "Nie, element posiada fabrycznie zarobione przewody przyłączeniowe gotowe do podłączenia do zacisków śrubowych lub szybkozłączek WAGO.")
        ]
    ...
    # -------------------------------------------------------------------------
    # 3. PUSZKI MONTAŻOWE (PR-BOX, TM-BOX) I UCHWYTY DO PILOTÓW (FUT099)
    # -------------------------------------------------------------------------
    if "PR-BOX" in umcode or "TM-BOX" in ucode or "PUSZKA" in uname or ("UCHWYT" in uname and ("PILOT" in uname or "MILIGHT" in uname or "FUT099" in umcode or "FUT099" in ucode)):
        if "PUSZKA" in uname or "PR-BOX" in umcode or "TM-BOX" in ucode:
            ...
            p1_opts = [
                f"Puszka instalacyjna podtynkowa to solidny element osprzętu elektroinstalacyjnego stworzony do bezpiecznego i estetycznego osadzania szklanych paneli dotykowych oraz naściennych sterowników oświetlenia LED w ścianie.",
                f"Puszka montażowa ścienna marki {brand} stanowi dedykowaną bazę instalacyjną pod nowoczesne panele dotykowe i regulatory LED w standardowym formacie 86 x 86 mm."
            ]
            p2_opts = [
                "Zapewnia odpowiednią przestrzeń na wygodne ułożenie przewodów zasilających i sterujących, a liczne perforowane przepusty ułatwiają szybkie wprowadzenie peszli lub kabli bez ryzyka ich uszkodzenia.",
                "Konstrukcja wykonana ze wzmocnionego tworzywa termoplastycznego gwarantuje wysoką odporność na pękanie podczas dokręcania wkrętów montażowych i stabilne trzymanie panelu dotykowego w płaszczyźnie ściany."
            ]
            intro_p1 = clean_text_repetitions(pick(uid, "box_p1", p1_opts))
            intro_p2 = clean_text_repetitions(pick(uid, "box_p2", p2_opts))

            uses = [
                "Stosowana w instalacjach mieszkaniowych, biurowych i hotelowych. Idealna do montażu w ścianach murowanych, betonowych oraz lekkich ściankach z płyt gipsowo-kartonowych (G-K).",
                "Przeznaczona do montażu wewnątrz budynków – w salonach, sypialniach, korytarzach i gabinetach, gdzie montowane są ścienne panele sterowania światłem LED."
            ]
            usage = pick(uid, "box_use", uses)
            ...
            faq = [
                ("Do jakich paneli pasuje ta puszka?", "Puszka jest w pełni kompatybilna ze wszystkimi szklanymi panelami naściennymi i sterownikami dotykowymi w standardzie 86 x 86 mm (w tym Prescot oraz MiBoxer)."),
                ("W jakich ścianach można ją zamontować?", "Nadaje się do montażu w ścianach murowanych, betonowych oraz w zabudowach z płyt gipsowo-kartonowych (G-K).")
            ]

            sec_heading = pick(uid, "box_head", [
                "Pewna baza montażowa pod panele dotykowe",
                "Stabilne osadzenie sterowników ściennych LED",
                "Estetyczny i bezpieczny montaż podtynkowy"
            ])
            return render_full_structure(title, sec_heading, [intro_p1, intro_p2], usage, features, faq, "Sterowniki LED", uid)

    # -------------------------------------------------------------------------
    # 4. ZASILACZE MODUŁOWE PR-MAD (SMART AUTO 12V / 24V)
    # -------------------------------------------------------------------------
    if "PR-MAD" in ucode or "PR-MAD" in umcode or "SMART AUTO" in uname:
        ...
        p1_opts = [
            "Innowacyjny zasilacz modułowy LED Prescot z serii PR-MAD Smart Auto to transformator wyposażony w zaawansowaną technologię automatycznego rozpoznawania napięcia podłączonego obwodu.",
            "Zasilacz impulsowy LED Prescot PR-MAD Smart Auto to uniwersalny zasilacz nowej generacji, który automatycznie dopasowuje parametry pracy do podłączonego odbiornika LED."
        ]
        p2_opts = [
            "Dzięki autodetekcji Smart Auto zasilacz samoczynnie dopasowuje właściwe parametry pracy, eliminując błędy instalatorskie i gwarantując bezawaryjne, stabilne zasilanie diod LED.",
            "Wzmocniona obudowa perforowana z aluminium zapewnia znakomite odprowadzanie ciepła drogą naturalnej konwekcji, gwarantując cichą, bezwentylatorową pracę i wieloletnią bezawaryjność."
        ]
        intro_p1 = clean_text_repetitions(pick(uid, "mad_p1", p1_opts))
        intro_p2 = clean_text_repetitions(pick(uid, "mad_p2", p2_opts))

        uses = [
            "Dedykowany do zasilania taśm LED w sufitach podwieszanych, szafach sterowniczych, ciągach meblowych oraz profilach architektonicznych.",
            "Idealny wybór dla profesjonalnych instalatorów realizujących mieszane instalacje oświetleniowe bez konieczności magazynowania dwóch różnych typów zasilaczy."
        ]
        usage = pick(uid, "mad_use", uses)
        ...
        faq = [
            ("Jak działa technologia Smart Auto 12V/24V?", "Układ sterujący zasilacza bada rezystancję i charakterystykę podłączonego obwodu LED przy starcie i precyzyjnie załącza stabilizowane napięcie 12V lub 24V DC."),
            ("Czy zasilacz wymaga wentylatora?", "Nie, dzięki wysokiej sprawności energetycznej (>88%) chłodzenie odbywa się całkowicie pasywnie przez aluminiową obudowę.")
        ]

        sec_heading = pick(uid, "mad_head", [
            "Inteligentna autodetekcja napięcia 12V i 24V DC",
            "Uniwersalne zasilanie LED z technologią Smart Auto",
            "Stabilna i cicha praca z pełną ochroną instalacji"
        ])

        return render_full_structure(title, sec_heading, [intro_p1, intro_p2], usage, features, faq, "Zasilacze LED", uid)

    # -------------------------------------------------------------------------
    # 5. ZASILACZE SCHÄRFER 7Y HERMETYCZNE IP67 ORAZ DOPUSZKOWE / WTYCZKOWE
    # -------------------------------------------------------------------------
    if "SCHARFER" in uprod or "SCHARFER" in uname or "SCH-" in ucode or "SCH-" in umcode:
        ...
        p1_opts = [
            "Zasilacz hermetyczny LED Schärfer 7Y to bezkompromisowy transformator impulsowy w klasie szczelności IP67, objęty 7-letnią gwarancją producenta i stworzony do bezawaryjnej pracy w trudnych warunkach środowiskowych.",
            "Profesjonalny zasilacz zewnętrzny Schärfer 7Y IP67 z technologią Powermax Inside zapewnia precyzyjnie stabilizowane napięcie stałe w wilgotnych i zewnętrznych instalacjach LED."
        ]
        p2_opts = [
            f"Masywna obudowa z odlewanego ciśnieniowo aluminium pełni rolę wydajnego radiatora, utrzymując optymalną temperaturę kluczowych półprzewodników nawet przy 100% obciążeniu ciągłym.",
            f"Wypełnienie wnętrza specjalną żywicą poliuretanową zabezpiecza elektronikę przed wilgocią, drganiami oraz kondensacją pary wodnej, co gwarantuje najwyższy wskaźnik MTBF (średni czas bezawaryjnej pracy)."
        ]
        intro_p1 = clean_text_repetitions(pick(uid, "sch_p1", p1_opts))
        intro_p2 = clean_text_repetitions(pick(uid, "sch_p2", p2_opts))

        uses = [
            "Zaprojektowany do zasilania zewnętrznych taśm LED, naświetlaczy i modułów na elewacjach, w ogrodach, na tarasach oraz w strefach mokrych (łazienki, baseny, myjnie, sauny).",
            "Idealny do wymagających instalacji architektonicznych i przemysłowych pracujących na zewnątrz budynków przez 365 dni w roku."
        ]
        usage = pick(uid, "sch_use", uses)
        ...
        faq = [
            ("Czy zasilacz może pracować bezpośrednio na zewnątrz?", "Tak, klasa szczelności IP67 oraz aluminiowa obudowa gwarantują pełną odporność na deszcz, śnieg, mróz i upały."),
            ("Dlaczego seria Schärfer ma 7 lat gwarancji?", "Dzięki zastosowaniu komponentów przemysłowych najwyższej klasy oraz masywnej aluminiowej obudowy odprowadzającej ciepło, zasilacze Schärfer charakteryzują się wyjątkową żywotnością.")
        ]

        sec_heading = pick(uid, "sch_head", [
            "Przemysłowa niezawodność IP67 i 7 lat gwarancji",
            "Hermetyczna ochrona i maksymalna sprawność energetyczna",
            "Odporność na wilgoć, pył i skrajne temperatury"
        ])

        return render_full_structure(title, sec_heading, [intro_p1, intro_p2], usage, features, faq, "Zasilacze LED", uid)

    # Zasilacze dopuszkowe
    if "PUSZKI" in uname or "DOPUSZKOW" in uname or "DO PUSZKI" in uname:
        ...
        features = [
            ...
            "Format obudowy: Dopuszkowy (przystosowany do standardowych puszek fi 60 mm)",
        ...
        faq = [
            ("Czy ten zasilacz zmieści się w standardowej puszce fi 60 mm?", "Tak, kompaktowe wymiary zasilacza zostały zaprojektowane tak, aby bez trudu zmieścił się w standardowej puszce instalacyjnej fi 60 mm."),
            ("Jakie przewody posiada zasilacz?", f"Model posiada {cable_desc}.")
        ]
        sec_heading = "Kompaktowe zasilanie dopuszkowe IP67"
        return render_full_structure(title, sec_heading, [intro_p1, intro_p2], usage, features, faq, "Zasilacze LED", uid)

    # Zasilacze wtyczkowe / gniazdkowe USB
    if "GNIAZDKOW" in uname or "WTYCZKOW" in uname or "USB" in uname:
        ...
        faq = [
            ("Do jakich urządzeń pasuje ten zasilacz?", "Pasuje do wszystkich odbiorników i sterowników LED wymagających stabilizowanego zasilania niskonapięciowego.")
        ]
        sec_heading = "Stabilne i bezpieczne zasilanie wtyczkowe"
        return render_full_structure(title, sec_heading, [intro_p1, intro_p2], usage, features, faq, "Zasilacze LED", uid)

    # -------------------------------------------------------------------------
    # 6. PROFILE ALUMINIOWE KLUŚ & PRESCOT
    # -------------------------------------------------------------------------
    if cat_root == "Profile do taśm LED" or "PROFIL" in uname:
        ...
        if is_corner:
            intro_p1 = clean_text_repetitions(f"Profil aluminiowy narożny LED {brand_str} to precyzyjna listwa konstrukcyjna zaprojektowana do kierowania strumienia świetlnego pod kątem 45 stopni bezpośrednio na płaszczyznę roboczą.")
            ...
            intro_p1 = clean_text_repetitions(f"Architektoniczny profil aluminiowy wpuszczany LED {brand_str} to precyzyjna listwa do bezszwowego montażu podtynkowego i wpustowego w meblach oraz płytach gipsowo-kartonowych.")
            ...
            intro_p1 = clean_text_repetitions(f"Uniwersalny profil aluminiowy LED {brand_str} to nowoczesna oprawa nawierzchniowa oraz wydajny radiator chłodzący taśmy LED.")
        ...
        faq = [
            ("Dlaczego profil aluminiowy jest konieczny przy montażu taśmy LED?", "Aluminium pełni funkcję radiatora – odbiera ciepło wydzielane przez pracujące diody. Bez profilu taśma LED ulega przegrzaniu, co prowadzi do szybkiej utraty jasności i spadku żywotności."),
            ("Jak zamontować profil?", "W zależności od modelu profil montuje się na wkręty, za pomocą dedykowanych sprężyn wpustowych, uchwytów zatrzaskowych lub zawiesza na linkach stalowych.")
        ]

        return render_full_structure(title, sec_heading, [intro_p1, intro_p2], usage, features, faq, "Profile do taśm LED", uid)

    # -------------------------------------------------------------------------
    # 7. OSŁONY I KLOSZE DO PROFILI KLUŚ / PRESCOT
    # -------------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------------
    # 8. TAŚMY LED (DELUX, COB, PREMIUM, STANDARD, BREAD, KOLORY, ITD.)
    # -------------------------------------------------------------------------
    if cat_root == "Taśmy LED" or "TAŚMA" in uname or "TASMA" in uname:
        if any(w in uname for w in ["DWUSTRONNA", "PIANKOWA", "TERMOPRZEWODZĄCA", "KOSZULKI SILIKONOWEJ"]):
            ...
            faq = [
                ("Jak przygotować powierzchnię do klejenia?", "Powierzchnię należy dokładnie oczyścić, odtłuścić i osuszyć przed naklejeniem taśmy.")
            ]
        ...
        # -------------------------------------------------------------
        # ŚCISŁA TAKSONOMIA KOLORÓW I RZECZYWISTEGO ZASTOSOWANIA TAŚM
        # -------------------------------------------------------------
        ...
        # Budowa akapitów wstępnych (Warstwa 1 i 2)
    ...
    # -------------------------------------------------------------------------
    # 9. STEROWNIKI LED I OSPRZĘT STERUJĄCY
    # -------------------------------------------------------------------------
    ...
    if clean_desc:
        ...
        if sentences:
            ...
            intro_p2 = sentences[1] if len(sentences) > 1 else f"Produkt marki {prod or 'Prescot'} charakteryzuje się solidną konstrukcją, trwałością oraz pełną zgodnością z normami instalacyjnymi."
        ...
        intro_p2 = clean_text_repetitions("Starannie dobrane materiały i wysoka jakość wykonania zapewniają bezpieczną oraz długotrwałą eksploatację w nowoczesnych instalacjach oświetlenia elektrycznego i LED.")
    usage = "Przeznaczony do stosowania w profesjonalnych i domowych instalacjach oświetlenia elektrycznego i LED."
    features = [
        "Przeznaczenie: Kompletacja zgodnego systemu oświetlenia LED",
        "Wysoka precyzja wykonania i stabilność montażu",
        "Trwałość i bezpieczeństwo zgodne z europejskimi normami instalacyjnymi"
    ]
    faq = [
        ("Z jakimi elementami współpracuje ten produkt?", "Produkt jest w pełni zintegrowany z pozostałymi komponentami oferty Prescot.")
    ]
    sec_heading = "Profesjonalny komponent instalacji oświetleniowej"
    return render_full_structure(title, sec_heading, [intro_p1, intro_p2], usage, features, faq, cat_root or "Oświetlenie LED", uid)


def render_full_structure(title, sec_heading, intro_paragraphs, usage, features, faq, cat_root, uid):
    # Oczyść cechy ze śmieciowych prefiksów kodowych
    clean_features = []
    for f in features:
        s = str(f).strip()
        s_low = s.lower()
...
def main():
    ...
    print(f"📦 Przetwarzanie {len(prods)} produktów przez Master SEO Suite V14.0 (Tuned Shoper & Strict Color Taxonomy)...")

    new_seo_products = {}
    for p in prods:
        key = p.get("key") or f"ean:{p.get('ean')}"
        copy_data = build_unique_product_copy(p)
    ...
    print(f"💾 Zapisywanie do {DIST_SEO_PATH}...")
    with open(DIST_SEO_PATH, "w", encoding="utf-8") as f:
        json.dump(out_payload, f, ensure_ascii=False, indent=2)

    print(f"💾 Zapisywanie do {DATA_SEO_PATH}...")
    with open(DATA_SEO_PATH, "w", encoding="utf-8") as f:
        json.dump(out_payload, f, ensure_ascii=False, indent=2)

    print("✅ Zaktualizowano bazę opisów SEO w obu lokalizacjach.")
